...
def upload_file(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if len(sheets) > 1:
                messages.warning(request, f'{excel_file} has multi-sheet!')
                return render(request, 'home.html')

            # getting a only the first sheet(sheet 1)
            worksheet = wb["Sheet1"]
        ...
        if invalid_data:
            messages.warning(request, f'{excel_file} has invalid data format, Consider renaming the first sheet as "Sheet1"')
            return render(request, 'home.html')
...
def employees_report(request, ready):
    ...
    summary_ready = False
    # dd = Employee.objects.all().delete()
    # d = Access.objects.all().delete()
    if ready == 'False':
    ...
    for user in employees:
        ...
        for i in access:
            ...
            if (t.total_seconds() / 3600) >= 8:
                ...
                employee_last_out.ot = datetime.timedelta()
                user.ot += employee_last_out.ot
